...
int three_diff_frame(VideoCapture *stream_ptr, int delay_ms) {
	...
	read_3_gray_frames(stream_ptr, &frame1, &frame2, &frame3);
	...
	while (!stop) {
		...
		switch (cur_frame_position) {
			case 1:
				frame_read_status = read_1_gray_frame(stream_ptr, &frame1);
				break;
			case 2:
				frame_read_status = read_1_gray_frame(stream_ptr, &frame2);
				break;	
			case 3:
				frame_read_status = read_1_gray_frame(stream_ptr, &frame3);
				break;
		}
...
int read_1_gray_frame(VideoCapture *stream_ptr, Mat *frame) {
...
int read_3_gray_frames(VideoCapture *stream_ptr, 
Mat *frame1, Mat *frame2, Mat *frame3) {
	stream_ptr->read(*frame1);
	stream_ptr->read(*frame2);
	stream_ptr->read(*frame3);
